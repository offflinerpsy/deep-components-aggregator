...
def test_api_data():
    ...
    # Тест Search API
    try:
        response = requests.get("http://5.129.228.88:9201/api/search?q=LM317", timeout=10)
...
def test_product_api():
    ...
    try:
        response = requests.get("http://5.129.228.88:9201/api/product?src=mouser&id=LM317LID", timeout=10)
        if response.status_code == 200:
            data = response.json()
            if data.get("ok") and data.get("product"):
                product = data["product"]
                has_images = bool(product.get("images") and len(product["images"]) > 0)
                has_specs = bool(product.get("technical_specs") and len(product["technical_specs"]) > 0)
                has_pricing = bool(product.get("pricing") and len(product["pricing"]) > 0)

                log(f"✅ Product API: images={has_images}, specs={has_specs}, pricing={has_pricing}")
                log(f"   Данные: {product.get('title')}, изображений={len(product.get('images', []))}")
                log(f"   Спецификаций: {len(product.get('technical_specs', {}))}, цен: {len(product.get('pricing', []))}")

                if has_images and has_specs and has_pricing:
                    return True
                else:
                    log("⚠️ Product API: отсутствуют некоторые данные, но продолжаем тестирование")
                    return True  # Продолжаем даже если данные неполные
            else:
...
def browser_full_test():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.goto("http://5.129.228.88:9201/", wait_until="networkidle", timeout=30000)
            ...
            if search_input.count() > 0:
                ...
                page.goto("http://5.129.228.88:9201/?q=LM317", wait_until="networkidle")
                page.wait_for_timeout(3000)
                ...
                if results.count() > 0:
                    ...
                    page.goto("http://5.129.228.88:9201/product.html?src=mouser&id=LM317LID", wait_until="networkidle")
                    page.wait_for_timeout(3000)
...
def main():
    ...
    if not api_product_ok:
        log("❌ Product API тест не прошел, останавливаем тестирование")
        return False
    
    if not api_search_ok:
        log("⚠️ Search API не работает идеально, но продолжаем с Product API")
    ...
    if browser_ok:
        ...
        log("   - http://5.129.228.88:9201/ (главная)")
        log("   - http://5.129.228.88:9201/?q=LM317 (поиск)")
        log("   - http://5.129.228.88:9201/product.html?src=mouser&id=LM317LID (товар)")
        return True
    else:
