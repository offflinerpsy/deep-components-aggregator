...
print("=== Removing temp env vars ===")
ssh.exec_command(f"sed -i '/^DISABLE_CACHE=/d' {REMOTE_ROOT}/.env")
ssh.exec_command(f"sed -i '/^DISABLE_TME_PROXY=/d' {REMOTE_ROOT}/.env")
...
ssh.close()
print("\n✅ DONE! All fixes deployed with cache and proxy enabled")
