...
sftp = ssh.open_sftp()
sftp.put('server.js', f'{REMOTE_ROOT}/server.js')
sftp.close()

print("=== Setting DISABLE_CACHE=1 ===")
ssh.exec_command(f"sed -i '/^DISABLE_CACHE=/d' {REMOTE_ROOT}/.env || true")
ssh.exec_command(f"echo 'DISABLE_CACHE=1' >> {REMOTE_ROOT}/.env")
...
ssh.close()
print("\n✅ Done! Cache disabled, server restarted")
