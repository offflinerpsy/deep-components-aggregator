...
const MemoryIcon = () => (
  <svg className="w-12 h-12" viewBox="0 0 64 64" fill="none" stroke="#3498DB" strokeWidth="1.5">
    ...
    <line x1="26" y1="18" x2="26" y2="46" strokeWidth="0.5" />
    <line x1="32" y1="18" x2="32" y2="46" strokeWidth="0.5" />
    <line x1="38" y1="18" x2="38" y2="46" strokeWidth="0.5" />
        ...
        <div className="container mx-auto px-6 py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm">
            <div className="flex items-center gap-4">
              <p className="text-muted-foreground">© 2025 Components Aggregator</p>
              <span className="text-border">•</span>
              <p className="text-muted-foreground">Агрегатор поиска электронных компонентов</p>
            </div>

            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                <ActivityIcon className={`w-4 h-4 ${apiStatus === "online" ? "text-green-600" : "text-red-600"}`} />
                <span className={`text-sm ${apiStatus === "online" ? "text-green-600" : "text-red-600"}`}>
                  API: {apiStatus === "online" ? "онлайн" : "офлайн"}
                </span>
              </div>

              <button className="text-muted-foreground hover:text-foreground transition-colors">Политика</button>
            </div>
          </div>
        </div>
      </footer>
