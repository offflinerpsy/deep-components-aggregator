...
# 1) Базовые пакеты
apt-get update -y
apt-get install -y git nginx jq curl ca-certificates net-tools
...
# 6) ФОЛБЭК-РОУТ /api/search (если его нет)
if ! grep -q 'app.get("/api/search"' server.js; then
  echo "INFO: /api/search endpoint not found. Adding a fallback."
...
// __FALLBACK_SEARCH_OK__: Injected by prod-recover.sh
...
# Убиваем старый процесс, если он занимает порт
PID_ON_PORT=$(netstat -ltnp | grep :9201 | awk '{print $7}' | cut -d'/' -f1 || true)
if [ -n "$PID_ON_PORT" ]; then
  echo "INFO: Killing process ${PID_ON_PORT} on port 9201."
