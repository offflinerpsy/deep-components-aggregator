...
const config = loadConfig('promelec');

export async function parsePromelec(mpn) {
  ...
  const brand = $product('td:contains("Производитель:")').next('td').text().trim();
  const packageInfo = $product('td:contains("Корпус:")').next('td').text().trim();
  const packagingInfo = $product('td:contains("Тип упаковки:")').next('td').text().trim();
  
  // Описание
  const description = $product('h2:contains("Описание")').next('p, div').text().trim();
  
  // Технические параметры
  const specs = {};
  $product('h2:contains("Описание")').nextAll('table tr, .specs tr').each((i, row) => {
  ...
  // Документация
  const datasheets = [];
  $product('a:contains("Документация"), a:contains("Datasheet"), a[href$=".pdf"]').each((i, link) => {
