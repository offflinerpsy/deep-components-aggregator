 ...
 * Automatically uses globalDispatcher (WARP proxy if configured)
 */
import { fetch } from 'undici';
import { globalDispatcher } from '../net/dispatcher.mjs';
...
export async function fetchWithRetry(url, options = {}, retries = MAX_RETRIES) {
  ...
  const fetchOptions = {
    ...
    dispatcher: globalDispatcher  // Use WARP proxy if HTTP_PROXY is set
  };
  ...
  try {
    ...
    clearTimeout(timeoutId);
    // Enhanced logging for network-level failures to diagnose provider regressions
